...
export default function ProductsScreen() {
  ...
  const handleSaveClick = (index) => {
    ...
    instance.patch(`/api/products/${updatedProduct._id}`, updatedProduct).then(
      (response) => {
  ...
  const handleDeleteClick = (index) => {
    ...
    if (confirmation) {
      ...
      setPeople(newPeople);
      // console.log(_id);
      instance.delete(`/api/products/${_id}`).then(
        response => {
          console.log(response.data);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      instance.get('/api/products').then(
        response => {
  ...
  const handlerAddButton = async (event) => {
    ...
    try {
      const response = await instance.post('/api/products/addProduct', newProduct);
    ...
    <div className="px-4 sm:px-6 lg:px-40 lg:pb-20 lg:pt-10">
      <div className="sm:flex sm:items-center">
        ...
        <button
            ...
            className="inline-flex items-center mr-10 justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto"
          >
            Добавить скидку
          </button>
          <button
