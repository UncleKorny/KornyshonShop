...
function App() {
  ...
  //     baseURL: 'http://localhost:5000',
  //   });
  //   instance.get('/api/statistics').then(
  //     response => {
  ...
  const handlerSaveClick = (e) => {
    ...
    const newFilteredProducts = products.filter(
      (product) => Number(product.price) >= minPriceValue && Number(product.price) <= maxPriceValue && product.filter === filterSelect
    );
  ...
  const handlerSearchChange = async(e)=>{
    ...
    try{
      const response = await instance.get(`/api/products/search?query=${value}`);
